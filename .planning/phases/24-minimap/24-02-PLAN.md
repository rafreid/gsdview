...
must_haves:
  ...
  key_links:
    - from: "minimap canvas click"
      ...
      via: "minimapToWorld converts click coords, then cameraPosition animates"
      pattern: "minimapToWorld.*cameraPosition|navigateToMinimapPosition"
    - from: "applyIncrementalUpdate"
      to: "calculateMinimapBounds"
      via: "Bounds recalculation happens inside renderMinimap RAF loop"
...
function navigateToMinimapPosition(canvasX, canvasY) {
  // Convert canvas position to world coordinates using minimapToWorld from Plan 01
  const worldPos = minimapToWorld(canvasX, canvasY);

  // Calculate appropriate camera distance based on current graph bounds
  ...
  // Use a zoom level that shows good context (30% of graph span)
  const distance = maxSpan * 0.3;

  if (is3D) {
    // 3D mode: position camera at angle looking at clicked point
    ...
    // 2D mode: position camera directly above clicked point
    Graph.cameraPosition(
...
function navigateToMinimapPositionInstant(canvasX, canvasY) {
  ...
  if (is3D) {
    Graph.cameraPosition(
      ...
      { x: worldPos.x, y: worldPos.y, z: 0 },
      0  // Instant for drag (0ms duration)
    );
  } else {
    Graph.cameraPosition(
      { x: worldPos.x, y: worldPos.y, z: distance + 100 },
